...
load_image(void)
{
  ...
  if (type == 'm' && len > sizeof(Elf32_Ehdr)
      && BOOTABLE_I86_ELF((*((Elf32_Ehdr *)buffer))))
    {
	...
	{
	  print(" linux 'zImage' kernel too big, try 'make bzImage'\n");
	  errnum = ERR_WONT_FIT;
	  return 0;
	}

      print("   [Linux-%s, setup=0x%lx, size=0x%lx]\n",
  ...
  if (exec_type)  /* can be loaded like a.out */
	...
	str = "-and-data";
      ...
      print(", loadaddr=0x%x, text%s=0x%lx", cur_addr, str, text_len);
	  ...
	  && pu.aout->a_syms < (filemax - filepos))
	{
	  ...
	  *(((int *)cur_addr)++) = pu.aout->a_syms;
	  ...
	  print(", symtab=0x%lx", pu.aout->a_syms);
      ...
      for (i = 0; i < pu.elf->e_phnum; i++)
	{
	  ...
	  if (phdr->p_type == PT_LOAD)
		...
		cur_addr = memaddr+memsiz;
	      print(", <0x%x:0x%x:0x%x>", memaddr, filesiz,
		     memsiz-filesiz);
...
load_module(void)
{
  ...
  print("   [Multiboot-module @ 0x%x, 0x%x bytes]\n", cur_addr, len);
...
load_initrd(void)
{
  ...
  print("   [Linux-initrd @ 0x%lx, 0x%x bytes]\n", moveto, len);
