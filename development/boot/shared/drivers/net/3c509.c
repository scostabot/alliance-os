...
$Id: 3c509.c,v 1.1.1.1 1999/12/09 14:14:39 edwin Exp $
...
#ifdef EDEBUG
	print("{l=%d,t=%x}",s+ETHER_HDR_SIZE,t);
#endif
...
eeprom_rdy()
{
	...
	if (i >= MAX_EEPROMBUSY) {
	        /* print("3c509: eeprom failed to come ready.\r\n"); */
		print("3c509: eeprom is busy.\r\n"); /* memory in EPROM is tight */
		return (0);
	}
	return (1);
}
...
struct nic *t509_probe(struct nic *nic, unsigned short *probe_addrs)
{
	...
	for (failcount=0; failcount<500; failcount++) {
	...
	*/
	...
	if(eth_nic_base >= EP_EISA_START) {
		print("3C5x9 board on EISA at 0x%x - ",eth_nic_base);
	} else {
		print("3C5x9 board on ISA at 0x%x - ",eth_nic_base);
	}
